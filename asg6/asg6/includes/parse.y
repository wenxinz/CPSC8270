...
%token LESSEQUAL LPAR LSQB MINEQUAL MINUS 
%token<id> NAME 
%token NEWLINE 
%token<node> NOT 
%token NOTEQUAL 
...
%type<intNumber> augassign pick_PLUS_MINUS pick_multop pick_unop comp_op star_EQUAL
%type<node> term factor power test opt_test or_test and_test not_test
%type<node> atom expr_stmt expr xor_expr and_expr shift_expr arith_expr print_stmt 
%type<node> testlist pick_yield_expr_testlist opt_yield_test pick_yield_expr_testlist_comp
%type<node> yield_expr lambdef comparison testlist_comp suite fpdef opt_EQUAL_test 
%type<node> stmt simple_stmt compound_stmt small_stmt funcdef if_stmt flow_stmt return_stmt
%type<node> plus_stmt star_SEMI_small_stmt star_ELIF pick_argument argument
%type<vec> trailer star_trailer arglist opt_arglist varargslist star_fpdef_COMMA parameters star_argument_COMMA
...
fpdef // Used in: varargslist, star_fpdef_COMMA, fplist, star_fpdef_notest
	: NAME { $$ = new IdentNode($1,args); pool.add($$); delete [] $1;}
	...
	: small_stmt star_SEMI_small_stmt SEMI NEWLINE
      { 
        if($2 == nullptr){
          ...
          $$ = new SuiteNode($1);
          pool.add($$);
          static_cast<SuiteNode *>($$)->addStmtAtEnd($2);
        }
      }
	...
	: star_SEMI_small_stmt SEMI small_stmt
      {
        if($1 == nullptr){ 
          ...
          $$ = $1;
          static_cast<SuiteNode *>($$)->addStmtAtEnd($3);
        } 
      }
	| %empty {$$ = nullptr;}
	;
...
expr_stmt // Used in: small_stmt
	: testlist augassign pick_yield_expr_testlist
	  {
	    switch($2){
          case 1: $$ = new AddAsgBinaryNode($1, $3);break;
          case 2: $$ = new SubAsgBinaryNode($1, $3);break;
          case 3: $$ = new MulAsgBinaryNode($1, $3);break;
          case 4: $$ = new DivAsgBinaryNode($1, $3);break;
          case 5: $$ = new ModAsgBinaryNode($1, $3);break;
          case 6: $$ = new FloorDivAsgBinaryNode($1, $3);break;
	    }
	    pool.add($$);
	  }
	| testlist star_EQUAL
          { 
            if($2 == 0){
              ...
              Node * res = asgChain.top();
              asgChain.pop();
              Node * temp;
              temp = new AsgBinaryNode($1,res);
              pool.add(temp);
              $$ = new SuiteNode(temp);
              pool.add($$);
              while(!asgChain.empty()){
                 temp = new AsgBinaryNode(asgChain.top(),res);
                 pool.add(temp);
                 static_cast<SuiteNode *>($$)->addStmtAtEnd(temp);
                 asgChain.pop();
              }
            }
          } 
...
if_stmt // Used in: compound_stmt
	: IF test COLON suite star_ELIF ELSE COLON suite
      {
        if($5) static_cast<SuiteNode *>($4)->addStmtAtEnd($5);
        $$ = new IfNode($2,$4,$8);
        pool.add($$);
      }
...
plus_stmt // Used in: suite, plus_stmt
	: plus_stmt stmt
	  {
	    static_cast<SuiteNode *>($1)->addStmtAtEnd($2);
	  }
	| stmt
	  { 
	    $$ = new SuiteNode($1);
...
comparison // Used in: not_test, comparison
	...
	| comparison comp_op expr
      {
        switch($2){
          case 1: $$ = new LessBinaryNode($1,$3);break;
          case 2: $$ = new GreaterBinaryNode($1,$3);break;
          case 3: $$ = new EqualBinaryNode($1,$3);break;
          case 4: $$ = new GreaterEqBinaryNode($1,$3);break;
          case 5: $$ = new LessEqBinaryNode($1,$3);break;
          case 6: $$ = new NotEqBinaryNode($1,$3);break;
        }
        pool.add($$);
      }
	;
...
arith_expr // Used in: shift_expr, arith_expr
	...
	| arith_expr pick_PLUS_MINUS term 
      { 
        switch($2){
         case 1: $$ = new AddBinaryNode($1,$3);break;
         case 2: $$ = new SubBinaryNode($1,$3);break;
        }
        pool.add($$);
      }
	;
...
term // Used in: arith_expr, term
	...
	| term pick_multop factor
	  {
	   switch($2){
        case 1: $$ = new MulBinaryNode($1, $3);break;
        case 2: $$ = new DivBinaryNode($1, $3);break;
        case 3: $$ = new ModBinaryNode($1, $3);break;
        case 4: $$ = new FloorDivBinaryNode($1, $3);break;
	   }
	   pool.add($$);
	  }
	;
...
factor // Used in: term, factor, power
	: pick_unop factor
      {
       switch($1){
        case 1: $$ = new PlusUnaryNode($2);break;
        case 2: $$ = new MinusUnaryNode($2);break;
       }
       pool.add($$);
      }
	| power
...
power // Used in: factor
	...
	| atom star_trailer
          { if($2){
              $$ = new FuncCallNode(static_cast<IdentNode *>($1)->getIdent(),*$2);
              pool.add($$);
              delete $2;        
            }
            else { $$ = $1;}
...
lambdef // Used in: test
	: LAMBDA varargslist COLON test { $$ = nullptr;}     
	| LAMBDA COLON test { $$ = nullptr;}   
	;
...
arglist // Used in: opt_arglist
	: star_argument_COMMA pick_argument
          { if(!$1) { $$ = new std::vector<Node *>();$$->push_back($2);}
            else { $1->push_back($2);$$ = $1;}
          }
	;
