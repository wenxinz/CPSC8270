...
%type<intNumber> augassign pick_PLUS_MINUS pick_multop pick_unop star_trailer trailer comp_op
%type<node> star_EQUAL term factor power test opt_test or_test and_test not_test
...
%type<node> yield_expr lambdef comparison testlist_comp suite 
%type<node> stmt simple_stmt compound_stmt small_stmt funcdef if_stmt flow_stmt return_stmt
%type<node> plus_stmt star_SEMI_small_stmt star_ELIF
%start start
...
funcdef // Used in: decorated, compound_stmt
	: DEF NAME parameters COLON suite
	  { $$ = new FuncNode($2,$5); pool.add($$);delete [] $2;}
	;
	...
	: star_EQUAL EQUAL pick_yield_expr_testlist
	  {
        if($1 == nullptr ){
          ...
          asgChain.push(new AsgBinaryNode($1,$3));
          $$ = $3;
        }
	  }
	| %empty { $$ = nullptr;}
...
power // Used in: factor
	...
	| atom star_trailer
      { 
        switch($2){
        case 0: $$ = $1;break;
        case 1: $$ = new FuncCallNode(static_cast<IdentNode *>($1)->getIdent());pool.add($$);break;        
        }
      }
	;
